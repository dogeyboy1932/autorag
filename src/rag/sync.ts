...
create policy own_sources on sources for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy own_chunks on chunks for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy own_deletions on deletions for all using (user_id = auth.uid()) with check (user_id = auth.uid());
